...
export function KanbanBoard() {
  const columns = useTaskStore((state: { columns: any }) => state.columns);
  const setColumns = useTaskStore((state: { setCols: any }) => state.setCols);
  const pickedUpTaskColumn = useRef<
    ColumnId | string | number | 'TODO' | 'IN_PROGRESS' | 'DONE'
  >('TODO');
  const columnsId = useMemo(
    () => columns.map((col: { id: any }) => col.id),
    [columns]
  );

  const tasks = useTaskStore((state: { tasks: any }) => state.tasks);
  const setTasks = useTaskStore((state: { setTasks: any }) => state.setTasks);
  ...
  function getDraggingTaskData(taskId: UniqueIdentifier, columnId: ColumnId) {
    const tasksInColumn = tasks.filter(
      (task: { status: string }) => task.status === columnId
    );
    const taskPosition = tasksInColumn.findIndex(
      (task: { id: UniqueIdentifier }) => task.id === taskId
    );
          ...
          {columns?.map((col: Column, index: number) => (
            <Fragment key={col.id}>
              <BoardColumn
                column={col}
                tasks={tasks.filter(
                  (task: { status: any }) => task.status === col.id
                )}
              />
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    const activeColumnIndex = columns.findIndex(
      (col: { id: UniqueIdentifier }) => col.id === activeId
    );

    const overColumnIndex = columns.findIndex(
      (col: { id: UniqueIdentifier }) => col.id === overId
    );
  ...
  function onDragOver(event: DragOverEvent) {
    ...
    if (isActiveATask && isOverATask) {
      const activeIndex = tasks.findIndex(
        (t: { id: UniqueIdentifier }) => t.id === activeId
      );
      const overIndex = tasks.findIndex(
        (t: { id: UniqueIdentifier }) => t.id === overId
      );
