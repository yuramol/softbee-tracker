...
function generateHtmlReports(data) {
    let htmlTrackers = "";
    let totalTime = 0;
    data.trackersGroupDay.forEach((group) => {
        group.trackers.forEach((tracker, index) => {
            ...
            htmlTrackers += `<tr>
              ${index === 0 ? `<th rowspan="${group.trackers.length}" class="table-header">${(0, date_fns_1.format)(group.date, 'MM/dd/yyyy')}</th>` : ''}
              ...
              <td class="table-common-time">${(0, date_fns_1.format)(tracker.createdAt, 'MM/dd/yyyy HH:mm')}</td>
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrapper">
      ...
      <div class="container">
        <p>Period:</p>
        <p class="period">${(0, date_fns_1.format)(data.period.start, 'MM/dd/yyyy')} - ${(0, date_fns_1.format)(data.period.start, 'MM/dd/yyyy')}</p>
      </div>
    </div>
    <table>
      <thead>
