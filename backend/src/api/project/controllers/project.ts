  ...
  ({ strapi }) => ({
    async generatePdf(ctx) {
      try {
        ...
        const start =
          typeof ctx.query.start === 'string'
            ? ctx.query.start
            : format(new Date(), 'yyyy-MM-dd');
        const end =
          typeof ctx.query.end === 'string'
            ? ctx.query.end
            : format(new Date(), 'yyyy-MM-dd');
          ...
          'api::tracker.tracker',
          {
            filters: {
              ...
              date: {
                $between: [
                  format(
                    startOfDay(parse(start, 'yyyy-MM-dd', new Date())),
                    'yyyy-MM-dd HH:mm:ss',
                  ),
                  format(
                    endOfDay(parse(end, 'yyyy-MM-dd', new Date())),
                    'yyyy-MM-dd HH:mm:ss',
                  ),
                ],
              },
        ...
        const trackersGroupDay = trackers.reduce(
          (acc: TTrackerGroup[], tracker) => {
            const trackerDate = parse(tracker.date, 'yyyy-MM-dd', new Date());
            const trackerDuration = tracker.durationMinutes;
            const findTracker = acc.find(trackerGroupDay =>
              isEqual(trackerGroupDay.date, trackerDate),
            );
            if (findTracker) {
              findTracker.trackers.push({
                ...
                userName: `${tracker.user.firstName ?? ''} ${
                  tracker.user.lastName ?? ''
                }`,
